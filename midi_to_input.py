...
- When a MIDI NOTE ON is received for pitch 123, send a toggle message for mouse movement to the Tobii 4C OSC script via TCP and change the `mode` to `paused`
- When a MIDI NOTE ON is received for pitch 122, send a toggle message for mouse movement to the Tobii 4C OSC script via TCP and change the `mode` to `frozen`
...
class MidiInputController:
    ...
    """
    def __init__(self, mouse_controller, keyboard_controller, is_frozen_func, tcp_host="127.0.0.1", tcp_port=9003):
        self.mouse = mouse_controller
    ...
    def send_tobii_toggle(self):
        """Sends a toggle message for mouse movement to the Tobii 4C OSC script via TCP."""
        try:
            with socket.create_connection((self.tcp_host, self.tcp_port), timeout=2.0) as s:
                s.sendall(b"toggle\n")
            print(f"[Triggered] Sent mouse toggle message to Tobii 4C OSC at {self.tcp_host}:{self.tcp_port}")
            return True
        except Exception as e:
            print(f"[TCP Warning] Could not send toggle message to Tobii 4C OSC at {self.tcp_host}:{self.tcp_port}: {e}")
            return False
    ...
    def handle_midi_message(self, msg):
        ...
        if is_note_on:
            if pitch == 127:
                ...
                print(f"[Triggered] Tobii 4C TCP Toggle & change Mode to 'paused' (Pitch {pitch})")
                self.send_tobii_toggle()
                self.mode = "paused"
            elif pitch == 122:
                print(f"[Triggered] Tobii 4C TCP Toggle & change Mode to 'frozen' (Pitch {pitch})")
                self.send_tobii_toggle()
                self.mode = "frozen"
...
def auto_connect_midi(mido_module, preferred_port_name=None):
    """
    ...
    if preferred_port_name:
        try:
            return mido_module.open_input(preferred_port_name)
        except Exception as e:
    ...
    try:
        inport = mido_module.open_input(virtual_port_name, virtual=True)
    ...
    while True:
        ...
        if target_port:
            try:
                inport = mido_module.open_input(target_port)
...
def run_script():
    ...
    parser = argparse.ArgumentParser(description="MIDI to Mouse Clicks & Input Actions.")
    ...
    parser.add_argument("--tcp-host", type=str, default="127.0.0.1", help="TCP Host IP for Tobii 4C OSC script (default: 127.0.0.1).")
    parser.add_argument("--tcp-port", type=int, default=9003, help="TCP Port for Tobii 4C OSC script mouse toggle commands (default: 9003).")
    ...
    controller = MidiInputController(mouse, keyboard, monitor.is_frozen, tcp_host=args.tcp_host, tcp_port=args.tcp_port)

    # Connect to MIDI
    inport = auto_connect_midi(mido, args.port)

    print("\n" + "="*50)
    print(" MIDI To Input translation active!")
    print(f" - Mouse frozen threshold set to: {args.threshold} seconds")
    print(f" - Tobii 4C TCP Toggle Address: {args.tcp_host}:{args.tcp_port}")
